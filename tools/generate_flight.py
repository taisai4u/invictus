...
def quat_align_z_to(v: np.ndarray) -> np.ndarray:
    """Return quaternion that rotates body Z [0,0,1] to unit vector v."""
    dot = float(np.clip(v[2], -1.0, 1.0))
    if dot >= 1.0 - 1e-9:
        return np.array([1.0, 0.0, 0.0, 0.0])
    if dot <= -1.0 + 1e-9:
        return np.array([0.0, 1.0, 0.0, 0.0])  # 180° about X
    sin_theta = math.sqrt(1.0 - dot * dot)
    # cross([0,0,1], v) = (-v[1], v[0], 0)
    axis = np.array([-v[1], v[0], 0.0]) / sin_theta
    half = math.acos(dot) / 2.0
    s = math.sin(half)
    return np.array([math.cos(half), axis[0] * s, axis[1] * s, 0.0])
...
MAG_FIELD_WORLD = np.array(
    [20.0, -3.5, 45.0]
)  # microtesla, in world frame (X=N, Y=E, Z=Up-ish)
...
def pressure_from_altitude(alt_m: float) -> float:
    ...
    if alt_m < 0:
        alt_m = 0.0
    return SEA_LEVEL_PRESSURE * (1 - LAPSE_RATE * alt_m / SEA_LEVEL_TEMP) ** (
        G / (LAPSE_RATE * 287.05)
    )
...
class RocketParams:
    ...
    spin_rate_radps: float = 15.0  # initial spin rate about body Z (long axis)
...
class SensorNoise:
    ...
    gyro_bias: np.ndarray = field(
        default_factory=lambda: np.array([0.001, -0.0005, 0.0008])
    )
...
def simulate(
    ...
    spin_angle = 0.0
    omega = np.array([0.0, 0.0, params.spin_rate_radps])  # derived each step for gyro

    mass_rate = (
        params.mass_propellant / params.burn_time if params.burn_time > 0 else 0.0
    )
    ...
    for i in range(n_steps + 1):
        ...
        mass = (
            params.mass_dry
            + params.mass_propellant
            - mass_rate * min(t, params.burn_time)
        )
        ...
        if not landed:
            ...
            pos += vel * dt
            spin_angle += params.spin_rate_radps * dt
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate synthetic rocket flight data"
    )
    parser.add_argument(
        "-o", "--output", type=Path, default=Path("simulated_flight.csv")
    )
    parser.add_argument("--dt", type=float, default=0.001, help="Timestep in seconds")
    parser.add_argument(
        "--duration", type=float, default=30.0, help="Total sim duration in seconds"
    )
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--thrust", type=float, default=20.0, help="Motor thrust in N")
    parser.add_argument(
        "--burn-time", type=float, default=2.0, help="Burn time in seconds"
    )
    parser.add_argument(
        "--tilt", type=float, default=2.0, help="Initial tilt from vertical in degrees"
    )
    ...
    print(
        f"Simulating: {args.thrust:.0f}N for {args.burn_time:.1f}s, "
    ...
    est_apogee = (
        (1 - (min_pressure / SEA_LEVEL_PRESSURE) ** (1 / 5.2561))
        * SEA_LEVEL_TEMP
        / LAPSE_RATE
    )
